/*
 * Copyright (c) 2017  razorlt, piros Papadimitriou
 *
...
void XPT2046::begin(uint16_t width, uint16_t height) {
  ...
  SPI.setClockDivider(SPI_CLOCK_DIV4);	//Full speed @18MHz
...
void XPT2046::getRaw (uint16_t &vi, uint16_t &vj, adc_ref_t mode, uint8_t max_samples) const {
  ...
  SPI.transfer(0);  // Flush last read, just to be sure
  SPI.transfer(0);
...
void XPT2046::powerDown() const {
  ...
  SPI.transfer(CTRL_HI_Y | CTRL_LO_SER);
  SPI.transfer(0);  // Flush, just to be sure
  SPI.transfer(0);
  digitalWrite(_cs_pin, HIGH);
}
